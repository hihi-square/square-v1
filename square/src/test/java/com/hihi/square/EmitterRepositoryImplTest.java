...
class EmitterRepositoryImplTest {
	...
	private Store store1 = Store.builder().usrId(1).password("1234").name("hi").build();
	...
	public void saveEventCache() throws Exception {
		...
		Notification notification = new Notification(store1, NotificationType.READY, "스터디 신청이 왔습니다.", "url",
			false);
	...
	public void findAllEmitterStartWithByMemberId() throws Exception {
		...
		//when
		Map<String, SseEmitter> ActualResult = emitterRepository.findAllEmitterStartWithByMemberId(
	...
	public void findAllEventCacheStartWithByMemberId() throws Exception {
		...
		Notification notification1 = new Notification(store1, NotificationType.READY, "스터디 신청이 왔습니다.", "url",
			false);
		emitterRepository.saveEventCache(eventCacheId1, notification1);

		Thread.sleep(100);
		String eventCacheId2 = memberId + "_" + System.currentTimeMillis();
		Notification notification2 = new Notification(store1, NotificationType.ACCEPT, "스터디 신청이 승인되었습니다.",
			"url", false);
		emitterRepository.saveEventCache(eventCacheId2, notification2);

		Thread.sleep(100);
		String eventCacheId3 = memberId + "_" + System.currentTimeMillis();
		Notification notification3 = new Notification(store1, NotificationType.REJECT, "스터디 신청이 거절되었습니다.",
			"url", false);
		emitterRepository.saveEventCache(eventCacheId3, notification3);

		//when
		Map<String, Object> ActualResult = emitterRepository.findAllEventCacheStartWithByMemberId(
			String.valueOf(memberId));
	...
	public void deleteAllEmitterStartWithId() throws Exception {
		...
		Assertions.assertEquals(0,
			emitterRepository.findAllEmitterStartWithByMemberId(String.valueOf(memberId)).size());
	}
	...
	public void deleteAllEventCacheStartWithId() throws Exception {
		...
		//then
		Assertions.assertEquals(0,
			emitterRepository.findAllEventCacheStartWithByMemberId(String.valueOf(memberId)).size());
	}
}