...
public class SaleController {
	...
	// 세일 등록
	@PostMapping("/sale")
	public ResponseEntity<?> createSale(Authentication authentication,
		...
		for (SaleMenuFormDto m : request.getMenus()) {
			...
			if (menu.getUser().getUsrId() != user.getUsrId()) {
				return new ResponseEntity<>(
					CommonResponseDto.builder().statusCode(400).message("CONTAIN_NO_AUTHORIZED_MENU").build(),
	...
	public ResponseEntity<?> finishSale(Authentication authentication, @PathVariable Integer saleId) {
		...
		if (!optionalSale.isPresent()) {
			return new ResponseEntity<>(CommonResponseDto.builder().message("BAD_SALE_ID").statusCode(400).build(),
				HttpStatus.BAD_REQUEST);
		}
		Sale sale = optionalSale.get();
		if (user.getUsrId() != sale.getStore().getUsrId()) {
			return new ResponseEntity<>(CommonResponseDto.builder().message("NO_AUTHORIZE").statusCode(400).build(),
				HttpStatus.BAD_REQUEST);
		}
		saleService.finishSale(sale);
		return new ResponseEntity<>(CommonResponseDto.builder().statusCode(200).message("SUCCESS_FINISH_SALE").build(),
			HttpStatus.OK);
	}
	...
	public ResponseEntity<?> getMyAllSale(Authentication authentication) {
		...
		User user = userService.findByUid(uid).get();
		// if (!(user instanceof Store)) {
		// 	return new ResponseEntity<>(CommonResponseDto.builder().statusCode(400).message("NO_AUTHORIZED").build(),
		// 		HttpStatus.BAD_REQUEST);
		// }
		List<StoreSaleDto> result = saleService.getStoreAllSale((Store)user);
		if (result.isEmpty()) {
			return new ResponseEntity<>(CommonResponseDto.builder().statusCode(200).message("NO_SALE").build(),
				HttpStatus.OK);
		} else {
			return new ResponseEntity<>(
				StoreAllSaleResponseDto.builder().statusCode(200).message("SUCCESS").sales(result).build(),
				HttpStatus.OK);
		}
	}
	...
	public ResponseEntity<?> getStoreAllSale(@PathVariable("id") Integer storeId) {
		...
		if (!optionalUser.isPresent()) {
			return new ResponseEntity<>(CommonResponseDto.builder().statusCode(400).message("NOT_EXISTS_STORE").build(),
				HttpStatus.BAD_REQUEST);
		}
	...
	public ResponseEntity<CommonResponseDto> deleteSale(Authentication authentication,
		@PathVariable("id") Integer saleId) {
		String uid = authentication.getName();
		User user = userService.findByUid(uid).get();
		// 로그인 회원이 가게 회원인지 확인
		// if (!(user instanceof Store)) {
		...
		// 세일이 존재하는지 확인
		if (!optionalSale.isPresent()) {
			return new ResponseEntity<>(CommonResponseDto.builder().statusCode(400).message("INVALID_SALE_ID").build(),
				HttpStatus.BAD_REQUEST);
		}
		Sale sale = optionalSale.get();
		// 로그인한 회원의 세일인지 확인
		if (user.getUsrId() != sale.getStore().getUsrId()) {
			return new ResponseEntity<>(
				CommonResponseDto.builder().statusCode(400).message("NO_AUTHORIZED_TO_DELETE").build(),
				HttpStatus.BAD_REQUEST);
		}
		saleService.deleteSale(sale);
		return new ResponseEntity<>(CommonResponseDto.builder().statusCode(200).message("SUCCESS_DELETE_SALE").build(),
			HttpStatus.OK);
	}

	// 세일 수정
	@PatchMapping("/sale")
	public ResponseEntity<CommonResponseDto> updateSale(Authentication authentication,
		@RequestBody @Valid SaleUpdateRequestDto request) {
		String uid = authentication.getName();
		User user = userService.findByUid(uid).get();
		// 로그인 회원이 가게 회원인지 확인
		// if (!(user instanceof Store)) {
		...
		if (user.getUsrId() != sale.getStore().getUsrId()) {
			return new ResponseEntity<>(
				CommonResponseDto.builder().statusCode(400).message("NO_AUTHORIZED_TO_UPDATE").build(),
				HttpStatus.BAD_REQUEST);
		}
		// 선택한 메뉴가 모두 해당 가게의 메뉴인지 확인
		for (SaleMenuFormDto m : request.getMenus()) {
		...
		return new ResponseEntity<>(CommonResponseDto.builder().statusCode(200).message("SUCCESS_UPDATE_SALE").build(),
			HttpStatus.OK);
	}
	...
	public ResponseEntity<?> getSaleDetail(@PathVariable("id") Integer saleId) {
		...
		return new ResponseEntity<>(
			StoreSaleDetailResponseDto.builder().sale(result).message("SUCCESS").statusCode(200).build(),
			HttpStatus.OK);
	}
	...
	public ResponseEntity<?> getEmdSaleList(@PathVariable("id") Integer emdId) {
		...
		if (emdAddressOptional.isEmpty()) {
			return new ResponseEntity<>(CommonResponseDto.builder().message("INVALID_EMD_ID").statusCode(400).build(),
				HttpStatus.BAD_REQUEST);
		}
