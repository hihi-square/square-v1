...
public class ReviewController {
	...
	public ResponseEntity writeReview(Authentication authentication, @RequestBody ReviewWriteRequestDto request) {
		...
		if (optionalOrder.isEmpty()) {
			return new ResponseEntity(CommonResponseDto.builder().statusCode(400).message("INVALID_ORDER_ID").build(),
		...
		if (!order.getCustomer().getUid().equals(uid)) {
			return new ResponseEntity(CommonResponseDto.builder().statusCode(400).message("NOT_USER_ORDER").build(),
				HttpStatus.BAD_REQUEST);
		}
		...
		if (order.getCreatedAt().isBefore(LocalDateTime.now().minusDays(5L))) {
			return new ResponseEntity(
				CommonResponseDto.builder().statusCode(400).message("CAN_WRITE_BEFORE_5DAYS").build(),
				HttpStatus.BAD_REQUEST);
		}
		// 리뷰가 이미 작성되었는지 확인
		if (reviewService.findByOrder(order).isPresent()) {
			return new ResponseEntity(
				CommonResponseDto.builder().statusCode(400).message("ALREADY_WRITE_REVIEW").build(),
				HttpStatus.BAD_REQUEST);
		}
		// 리뷰는 1 ~ 5만 가능
		if (request.getRating() < 1 || request.getRating() > 5) {
			return new ResponseEntity(CommonResponseDto.builder().statusCode(400).message("RATING_IS_1TO5").build(),
				HttpStatus.BAD_REQUEST);
		}

		reviewService.save((Customer)user, order, request);
		return new ResponseEntity(CommonResponseDto.builder().message("SUCCESS").statusCode(201).build(),
			HttpStatus.CREATED);
	...
	public ResponseEntity updateReview(Authentication authentication, @RequestBody ReviewUpdateRequestDto request) {
		...
		// if (!(user instanceof Customer)) {
		// 	return new ResponseEntity(
		// 		CommonResponseDto.builder().statusCode(400).message("ONLY_CUSTOMER_WRITE_REVIEW").build(),
		// 		HttpStatus.BAD_REQUEST);
		// }
		Optional<Review> optionalReview = reviewService.findById(request.getReviewId());
		// 유효하지 않은 리뷰 번호
		if (optionalReview.isEmpty()) {
			return new ResponseEntity(CommonResponseDto.builder().statusCode(400).message("INVALID_REVIEW_ID").build(),
				HttpStatus.BAD_REQUEST);
		}
		Review review = optionalReview.get();
		// 자신의 리뷰만 수정 가능
		if (!review.getCustomer().getUid().equals(uid)) {
			return new ResponseEntity(CommonResponseDto.builder().statusCode(400).message("NOT_MY_REVIEW").build(),
				HttpStatus.BAD_REQUEST);
		}
		// 리뷰는 1 ~ 5만 가능
		if (request.getRating() < 1 || request.getRating() > 5) {
	...
	public ResponseEntity getStoreReviews(@PathVariable("id") Integer storeId) {
		...
		// 	return new ResponseEntity(CommonResponseDto.builder().statusCode(400).message("ONLY_VIEW_STORE_ID").build(),
		// 		HttpStatus.BAD_REQUEST);
		// }
	...
	public ResponseEntity getMyReviews(Authentication authentication) {
		...
		return new ResponseEntity(CustomerReviewListResponseDto.builder().statusCode(200).reviews(reviews).build(),
			HttpStatus.OK);
	}
