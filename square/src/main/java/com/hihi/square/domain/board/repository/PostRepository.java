...
public interface PostRepository extends JpaRepository<Post, Integer> {
	...
	@Query("select p from Post p where p.board = :board and p.emdAddress in (:emdList)")
	List<Post> findByEmdAddressAndBoard(List<EmdAddress> emdList, Board board);

	@Query("select p from Post p where p.board = :board and p.emdAddress in (:emdList) and (p.title like CONCAT('%', :query, '%') OR p.content like CONCAT('%', :query, '%'))")
