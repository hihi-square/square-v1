...
public class CustomerAddressController {
	...
	//사용자 주소 등록
	@PostMapping
	public ResponseEntity<CommonResponseDto> createdCustomerAddress(Authentication authentication,
		@RequestBody @Valid CustomerAddressCreateRequestDto request) {
		...
		if (customerAddressService.findAllByCustomer(customer).size() >= 3) {
			return new ResponseEntity<>(CommonResponseDto.builder().message("FULL_ADDRESS").statusCode(400).build(),
				HttpStatus.BAD_REQUEST);
		}
		customerAddressService.addCustomerAddress(customer, request);
		return new ResponseEntity(CommonResponseDto.builder().statusCode(201).message("SUCCESS_ADD_ADDRESS").build(),
			HttpStatus.CREATED);
	}

	// 사용자 위치 삭제
	@DeleteMapping("/{id}")
	public ResponseEntity<?> deleteCustomerAddress(Authentication authentication,
		@PathVariable("id") Integer customerAddressId) {
		String uid = authentication.getName();
		// User user = userService.findByUid(uid).get();
		// 구매자만 사용자 주소 삭제 가능
		// if (!(user instanceof Customer)) {
		...
		if (!optionalCustomerAddress.isPresent()) {
			return new ResponseEntity(
				CommonResponseDto.builder().statusCode(400).message("INVALID_CUSTOMER_ADDRESS_ID").build(),
				HttpStatus.BAD_GATEWAY);
		}
		CustomerAddress customerAddress = optionalCustomerAddress.get();
		if (!customerAddress.getCustomer().getUid().equals(uid)) {
			return new ResponseEntity(
				CommonResponseDto.builder().statusCode(400).message("DELETE_ONLY_MY_ADDRESS").build(),
				HttpStatus.BAD_REQUEST);
		}
		customerAddressService.deleteAddress(customerAddress);
		return new ResponseEntity(CommonResponseDto.builder().message("SUCCESS").statusCode(200).build(),
			HttpStatus.OK);
	}
	...
	public ResponseEntity<?> getUsersCustomerAddressList(Authentication authentication) {
		...
		return new ResponseEntity<>(CustomerAddressListResponseDto.builder()
			.statusCode(200)
			.message("SUCCESS")
			.customerAddressDtoList(result)
			.build(), HttpStatus.OK);
	}
}
