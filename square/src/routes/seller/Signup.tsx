...
export default function Signup() {
  ...
  const handleSignUp = () => {
    ...
    const newForm = {
      ...
      longitude,
      businessInformation: {
        companyRegistrationNumber: rand,
        ceoName: "8조",
        openingDate: "2023-01-01",
        corporateRegistrationNumber: rand2,
        businessName: "가나다라마바사",
        businessFile: "dfkaslgtasjd",
      },
    };
  ...
  const onDataHandler = (address: Address) => {
    axios
      .get(
        `https://dapi.kakao.com/v2/local/search/address.json?query=${address.address}`,
        {
          headers: {
      ...
      .then((resp) => {
        console.log(resp);
        setBcode(resp.data.documents[0].address.b_code);
        setSaveAddress(resp.data.documents[0].road_address.road_name);
        setLongtitude(resp.data.documents[0].x);
        ...
        <Grid container xs={10}>
          ...
          <Grid xs={12} sx={{ backgroundColor: "white", marginBottom: "20px" }}>
            <div style={{ marginTop: "26px" }} className="inputTitle">
              주소 입력란
              <DaumPostcodeEmbed onComplete={onDataHandler} />
            </div>
