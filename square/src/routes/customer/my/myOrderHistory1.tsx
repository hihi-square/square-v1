                ...
                <Grid item xs={12} key={index}>
                    ...
                    <Box sx={{ padding: '1rem' }}>
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            ...
                            <IconButton onClick={() => setOpenProductIndex(openProductIndex === index ? null : index)}>
                                <ExpandMoreIcon />
                            </IconButton>
                        </Box>
                        <Collapse in={openProductIndex === index} style={{ maxHeight: '40px', overflow: 'auto' }}>
                            {order.products.map((product, pIndex) => (
                                <Box key={pIndex} sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', height: '15px', fontSize: '0.8rem' }}>
                                    <img src={product.thumbnail} alt={product.name} width="20" height="20" />
                        ...
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '0.5rem' }}>
                            ...
                            <IconButton onClick={() => setOpenPaymentIndex(openPaymentIndex === index ? null : index)}>
                                <ExpandMoreIcon />
                            </IconButton>
                        </Box>
                        <Collapse in={openPaymentIndex === index} style={{ maxHeight: '15px', overflow: 'auto' }}>
                            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', height: '15px', fontSize: '0.8rem' }}>
