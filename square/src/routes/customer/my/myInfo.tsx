import { Box, Grid, TextField, Typography, Avatar, IconButton, Button, Collapse } from '@mui/material';
...
function MyInfo() {
  ...
  const [user] = useState({
    profileImage: 'https://dummyimage.com/200x200', // Replace this with the actual profile image url
  ...
  return (
    <Box sx={{ padding: 3 }}>
      <Box sx={{ display: 'flex', alignItems: 'center', marginBottom: 2 }}>
        ...
        <Typography variant="h6" sx={{margin: 'auto'}}>내 정보 변경</Typography>
      </Box>

      <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', minHeight: '25vh', justifyContent: 'center', marginBottom: 2 }}>
        <Avatar src={user.profileImage} sx={{ width: 100, height: 100, mb: 2 }} />
        <IconButton color="primary" component="span" onClick={handleImageUpload}>
          <PhotoCamera />
        </IconButton>
      </Box>
      ...
      <Grid container spacing={2}>
        ...
        <Grid item xs={12}>
          ...
          <Collapse in={open}>
            <Box sx={{ border: '1px solid #ccc', borderRadius: 2, padding: 2, overflow: 'auto', maxHeight: 300 }}>
